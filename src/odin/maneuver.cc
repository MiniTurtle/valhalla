...
Maneuver::Maneuver()
    : type_(DirectionsLeg_Maneuver_Type_kNone), has_node_type_(false), traffic_signal_(false),
      is_steps_(false), is_bridge_(false), is_tunnel_(false), length_(0.0f), speed_limit_(0), speed_(0), time_(0), basic_time_(0),
...
void Maneuver::set_speed(uint32_t speed) {
    speed_ = speed;
}

uint32_t Maneuver::speed(const Options::Units& units) const {
    if (units == Options::miles) {
        return (speed_ * midgard::kMilePerKm);
    }
    return speed_;
}
